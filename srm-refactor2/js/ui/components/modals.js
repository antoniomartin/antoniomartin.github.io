...
export async function openContactoModal() {
    ...
    const optionsEmpresas = empresas.map(e => `<option value="${e.id}">${escapeHtml(e.nombre)}</option>`).join('');
    const optionsContactos = '<option value="">(Ninguno)</option>' + contactos.map(c => `<option value="${c.id}">${escapeHtml(c.nombre)} (${escapeHtml(empresas.find(e => e.id === c.empresaId)?.nombre || '')})</option>`).join('');
